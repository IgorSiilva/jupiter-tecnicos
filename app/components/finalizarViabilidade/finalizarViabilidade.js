...
import { StyleSheet, Image, ScrollView, View, Alert, AsyncStorage, NetInfo, StatusBar, ActivityIndicator, TouchableHighlight } from 'react-native'
import {
  Container,
  Content,
  Form,
...
import { salvarFoto, finalizarViabilidade, removerOS, removerFotos } from "../../helpers/databaseHelper"
...
class FinalizarViabilidade extends Component {
  ...
  onChangeText(name, value, somenteInteiro = false) {
    if (somenteInteiro)
      value = this.retirarPontoDeNumero(value)
    this.setState({
      [name]: value
    })
  }
    ...
    ImagePicker.launchCamera(options, (response) => {
      if (response.didCancel != true) {
        this.setState({
          ...
          return RNFS.unlink(response.path)
            .then(() => {
            ...
            finalizarViabilidade(dados, usuario).then(() => {
              salvarFoto(dados.ordem.idatendimento, this.state.imagens).then(() => {
                NetInfo.getConnectionInfo().then((statusNetwork) => {
                  if (statusNetwork.type == 'wifi' || statusNetwork.type == 'cellular') {
                    const db = SQLite.openDatabase({ name: "OS", createFromLocation: "1" });
                    db.transaction((tx) => {
                      tx.executeSql(`SELECT * FROM OS WHERE idordem = ${dados.ordem.idordem}`, [], (tx, results) => {
                        const OS = results.rows.raw()
                        db.transaction((tx) => {
                          tx.executeSql(`SELECT fotosViabilidade.* FROM OS LEFT JOIN fotosViabilidade ON OS.id = fotosViabilidade.id WHERE OS.finalizada = ${1} AND OS.id = ${dados.ordem.idatendimento}`, [], (tx, results) => {
                            const imagens = results.rows.raw()

                            try {
                              fetch(`${apiUrl}/api/action/OrdemDeServico/salvarAtendimentoOrdemDeServico`, {
                                method: "POST",
              ...
              <Form>
                ...
                <Item floatingLabel>
                  <Input
                    ...
                    onChangeText={value => this.onChangeText('esticadores', value, true)}
                  />
                </Item>
                ...
                <Item floatingLabel>
                  <Input
                    ...
                    onChangeText={historico => { this.setState({ historico }) }}
                  />
                </Item>
              </Form>
              <Button full primary style={{ marginVertical: 35 }} onPress={this.enviarFoto.bind(this)}>
              ...
              <View style={{ flex: 1, flexDirection: 'row', marginVertical: 10 }}>
                ...
                >
                  {this.state.imagens.map((imagem, index) => {
                    return (
                      <View key={index}>
                        <Image style={{ zIndex: 0, width: 150, height: 150, borderWidth: 1, borderColor: 'black', marginHorizontal: 5 }} source={{ uri: `data:image/gif;base64,${imagem}` }} />
                        <TouchableHighlight
                          style={{
                            position: 'absolute',
                            left: 0,
